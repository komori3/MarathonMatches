...
struct Path {
  ...
  void perturbate3() {
    if (points.front().distance(points.back()) == points.size() - 1) return;
    vector<int> cands;
...
struct State {
  ...
  void init() {
    ...
    memset(count, 0, sizeof(int) * 2500);
    memset(color, 0, sizeof(int) * 2500);
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
  ...
  void annealing() {
    ...
    while (timer.elapsedMs() < end) {
      ...
      double temp = get_temp(1.0, 0.1, timer.elapsedMs() - start, duration);
  ...
  int calc_score() const {
    static int colors[50][50];
    static ull used[50];
    calc_score_count++;
    vector<int> ccs(C, 0); // connected components
    //for (int i = 0; i < N; i++) {
    //  for (int j = 0; j < N; j++) {
    //    colors[i][j] = tile_colors[i][j][count[i][j] % C];
    //  }
    //}
    memcpy(colors, color, sizeof(int) * 2500);
    memset(used, 0, sizeof(ull) * 50);
  ...
  //  for (int i = 0; i < N - 1; i++) {
  //    for (int j = 0; j < N; j++) {
  //      if (colors[i][j] != colors[i + 1][j]) score++;
  //    }
  //  }
  //  for (int i = 0; i < N; i++) {
  //    for (int j = 0; j < N - 1; j++) {
  //      if (colors[i][j] != colors[i][j + 1]) score++;
  //    }
  //  }
  //  return score;
  //}
