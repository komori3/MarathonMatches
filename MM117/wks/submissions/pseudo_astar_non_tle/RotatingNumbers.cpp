...
namespace NRP {
  ...
  vector<BoardPtr> solve(const vector<int>& grid) {
    ...
    while (!pq.empty()) {
      ...
      if (b->loc_cost == 0 || timer.elapsedMs() > 9900) {
        vector<BoardPtr> ans;
        while (b != nullptr) {
          ans.push_back(b);
          b = b->parent;
...
struct Solver {
  ...
  void align(vector<int> p) {
    ...
    for(int n : p) {
      ...
      if (dst.x == N - 1) dst.y += 2;
      if (dst.y == N - 1) dst.x += 2;
...
int main() {
  ...
  //ifstream ifs("C:\\dev\\TCMM\\problems\\MM117\\seed2.txt");
  //istream& cin = ifs;
