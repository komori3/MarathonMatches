...
inline int get_distance(const Point& p1, const Point& p2) {
  return abs(p1.x - p2.x) + abs(p1.y - p2.y);
}

struct Square {
  int r, c, s;
  Square(int r, int c, int s) : r(r), c(c), s(s) {}
  inline bool is_inside(const Point& p) const {
    int y = p.y, x = p.x;
    return r <= y && y < r + s && c <= x && x < c + s;
  }
  Point cw(const Point& p) const {
    int y = p.y - r, x = p.x - c;
    return Point(r + x, c + s - y - 1);
  }
  Point ccw(const Point& p) const {
    int y = p.y - r, x = p.x - c;
    return Point(r + s - x - 1, c + y);
  }
  string toString() const {
    return "{" + to_string(r) + "," + to_string(c) + "," + to_string(s) + "}";
  }
...
struct State2 {
  ...
  int loc_penalty;
  int sdist;
  Point now1, now2, dst1, dst2;
  // taboo は外から受け取る
  State2(
    int N, int P, const vector<vector<int>>& g, int move_penalty, int loc_penalty,
    int sdist, const Point& now1, const Point& now2, const Point& dst1, const Point& dst2
  )
    : N(N), P(P), g(g), move_penalty(move_penalty), loc_penalty(loc_penalty),
    sdist(sdist), now1(now1), now2(now2), dst1(dst1), dst2(dst2) {}
  ...
  int calc_loc_penalty(int r, int c, int s) const {
    ...
    for (int y = r; y < r + s; y++) {
      for (int x = c; x < c + s; x++) {
        int k = g[r][c];
        loc_penalty += abs(y - k / N) + abs(x - k % N);
      }
    }
    return loc_penalty;
  }
  ...
  void add_next_states(int width, vector<DEPQ<shared_ptr<State2>>>& pqs, const vector<vector<bool>>& taboo) {
    int dist = get_distance(now1, dst1) + get_distance(now2, dst2);
    Point prev1 = now1, prev2 = now2;
    for (int s = 2; s <= 2; s++) {
      // dy, dx: オフセットからの位置
      for (int dy = 0; dy < s; dy++) {
        for (int dx = 0; dx < s; dx++) {
          ...
          int oy = now1.y - dy, ox = now1.x - dx;
          //dump(dy, dx, oy, ox, s);
          // inside
          if (!is_inside(oy, ox) || !is_inside(oy + s - 1, ox + s - 1)) continue;
          Square sq(oy, ox, s);
          if (!sq.is_inside(now2)) continue;
          // taboo
          bool ok = true;
          ...
          if (!ok) continue;
          // left move
          Point lp1 = sq.ccw(now1), lp2 = sq.ccw(now2);
          // dist を減らす(近づく)もののみ許容
          int ndist;
          ndist = get_distance(dst1, lp1) + get_distance(dst2, lp2);
          if (ndist < dist) {
            move(oy, ox, s, 'L');
            now1 = lp1;
            now2 = lp2;
            if (pqs[sdist - ndist].size() < width) {
              shared_ptr<State2> nstate = make_shared<State2>(*this);
          ...
          Point rp1 = sq.cw(now1), rp2 = sq.cw(now2);
          ndist = get_distance(dst1, rp1) + get_distance(dst2, rp2);
          if (ndist < dist) {
            move(oy, ox, s, 'R');
            now1 = rp1;
            now2 = rp2;
...
using State2Ptr = shared_ptr<State2>;

bool operator<(const State2Ptr& a, const State2Ptr& b) {
    ...
    for (int s = 2; s <= N; s++) {
      ...
      for (int dy = 0; dy < s; dy++) {
        for (int dx = 0; dx < s; dx++) {
          ...
          int ndist;
          ndist = abs(dst.y - lp.y) + abs(dst.x - lp.x);
          if (ndist == dist - (s - 1)) {
            move(oy, ox, s, 'L');
            now = lp;
            if (pqs[sdist - ndist].size() < width) {
          ...
          Point rp = sq.cw(now);
          ndist = abs(dst.y - rp.y) + abs(dst.x - rp.x);
          if (ndist == dist - (s - 1)) {
            move(oy, ox, s, 'R');
            now = rp;
            if (pqs[sdist - ndist].size() < width) {
...
struct Solver {
  ...
  void move_to(const Point& dst, const Point& src, const vector<vector<bool>>& taboo) {
    static int bws[41] = {
      0,
  ...
  void move_to_double(const Point& dst1, const Point& dst2, const Point& src1, const Point& src2, const vector<vector<bool>>& taboo) {
    using PQ = DEPQ<State2Ptr>;
    int sdist = get_distance(dst1, src1) + get_distance(dst2, src2);
    State2Ptr init_state = make_shared<State2>(N, P, g, move_penalty, loc_penalty, sdist, src1, src2, dst1, dst2);
    vector<PQ> pqs(sdist + 1);
    vector<State2Ptr> bests(sdist + 1, nullptr);
    pqs[0].push(init_state);
    bests[0] = init_state;
    int width = 3;
    for (int d = 0; d < sdist; d++) {
      for (int w = 0; w < width && !pqs[d].empty(); w++) {
        const auto& now_state = pqs[d].get_max();
        if (bests[d] == nullptr || now_state->get_penalty() < bests[d]->get_penalty()) {
          bests[d] = now_state;
        }
        now_state->add_next_states(width, pqs, taboo);
        pqs[d].pop_max();
    ...
    if (!pqs[sdist].empty()) {
      bests[sdist] = pqs[sdist].get_max();
    }
    for (int d = sdist; d >= 0; d--) {
      if (bests[d] != nullptr) {
        const State2Ptr& best_state = bests[d];
        for (const Move& m : best_state->moves) {
          //dump(m);
          move(m);
        }
  ...
  void move_to_simple(const Point& dst, const Point& src, const vector<vector<bool>>& taboo) {
    static int bws[41] = {
      0,
      0,0,0,500,500,500,500,500,500,500,
      200,200,200,200,200,100,100,100,100,100,
    ...
    StatePtr init_state = make_shared<State>(N, P, g, move_penalty, loc_penalty, src, dst);
    vector<PQ> pqs(sdist + 1);
    vector<StatePtr> bests(sdist + 1, nullptr);
    pqs[0].push(init_state);
    bests[0] = init_state;
    int width = bws[N];
    for (int d = 0; d < sdist; d++) {
    ...
    for (int d = sdist; d >= 0; d--) {
      if (bests[d] != nullptr) {
        const StatePtr& best_state = bests[d];
        for (const Move& m : best_state->moves) {
          move(m);
        }
        return;
  ...
  Point get_triangle_point(const Point& p1, const Point& p2, const Point& p3) const {
    auto get_dist = [](const Point& a, const Point& b) {
      return abs(a.x - b.x) + abs(a.y - b.y);
    };
    int dist = INT_MAX, r = -1, c = -1;
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
        Point p(i, j);
        int d = get_dist(p1, p) + get_dist(p2, p) + get_dist(p3, p);
        if (d < dist) {
          dist = d;
          r = i;
          c = j;
  ...
  void align_double(int n1, int n2, vector<vector<bool>>& taboo) {
    Point dst1(n1 / N, n1 % N), dst2(n2 / N, n2 % N);
    Point now1 = get_pos(n1), now2 = get_pos(n2);
    Point p = get_triangle_point(dst1, now1, now2);
    if (p == now1) {
      taboo[p.y][p.x] = true;
      move_to_simple(p, now2, taboo);
      now2 = get_pos(n2);
    }
    else if (p == now2) {
      taboo[p.y][p.x] = true;
      move_to_simple(p, now1, taboo);
      now1 = get_pos(n1);
    }
    else {
      move_to_simple(p, now1, taboo);
    ...
    assert(abs(now1.x - now2.x) + abs(now1.y - now2.y) == 1);
    taboo[p.y][p.x] = false;
    move_to_double(dst1, dst2, now1, now2, taboo);
    now1 = get_pos(n1);
    move_to_simple(dst1, now1, taboo);
    taboo[dst1.y][dst1.x] = true;
    now2 = get_pos(n2);
    move_to_simple(dst2, now2, taboo);
    taboo[dst2.y][dst2.x] = true;
  }
  ...
  void align(vector<int> p) {
    ...
    for (int k = 0; k < p.size(); k++) {
      ...
      if (i < N - 2 && j < N - 2) {
        int n1 = n, n2 = p[k + 1];
        Point dst1(n1 / N, n1 % N), dst2(n2 / N, n2 % N), p1(get_pos(n1)), p2(get_pos(n2)), t(get_triangle_point(dst1, p1, p2));
        int s1 = get_distance(dst1, p1) + get_distance(dst2, p2);
        int s2 = get_distance(dst1, t) + get_distance(t, p1) + get_distance(t, p2);
        if (i < N - 3 && j < N - 3 && s2 < s1 - 30) {
          //dump(n1, n2);
          align_double(n1, n2, taboo);
          k++;
        }
        else {
...
int main() {
  ...
  //ifstream ifs("C:\\dev\\TCMM\\problems\\MM117\\seed3.txt");
  //istream& cin = ifs;
