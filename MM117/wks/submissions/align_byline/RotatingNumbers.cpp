...
struct Solver {
  ...
  void align_default(int n, Point& pos, const Point& target) {
    //dump(pos, target);
    // 最下段にある場合は一段上げる
    if (pos.y == N - 1) {
      //dump("hoge");
      //print();
      if (pos.x == N - 1) {
        move(pos.y - 1, pos.x - 1, 2, 'L');
      }
      else {
        move(pos.y - 1, pos.x, 2, 'R');
      }
      pos.y--;
      //print();
    }
    // 左側にある場合、target.x まで右にずらす
    while (pos.x < target.x) {
      //dump("hoge");
      //print();
      move(pos.y, pos.x, 2, 'R');
      pos.x++;
      //print();
    }
    // 最右端にある場合、左に一つずらす
  ...
  void align_edge(int n, Point& pos, const Point& target) {
    // 最下端にある場合、引き上げ
    //dump(n, pos, target);
    if (pos.y == N - 1) {
      if (pos.x == N - 1) {
    ...
    if (pos.y == target.y + 1 && (pos.x == target.x - 1 || pos.x == target.x)) {
      // pos が (target.y + 1, target.x - 1) or (target.y + 1, target.x) の場合
      // 下に退避
      move(pos.y, pos.x - 1, 2, 'R');
      pos.y++;
    }
    // (target.y, target.x - 1, 'R')
    move(target.y, target.x - 1, 2, 'R');
    // (target.y + 1, target.x) に持っていく
    // 最右端に運ぶ
    while (pos.x < N - 1) {
      move(pos.y, pos.x, 2, 'R');
      pos.x++;
    ...
    //// 最後に L
    //move(target.y, target.x - 1, 2, 'L');
    //pos.y--;
  }
  void align(int n) {
    //dump(n);
    auto pos = get_pos(n);
    auto target = get_target(n);
    if (target.y >= N - 2) return;
    if (target.x == N - 1) {
      if (target.y != N - 3) {
        align_edge(n, pos, target);
      }
    }
    else {
      align_default(n, pos, target);
    }
  }
};
