...
int main() {
  ...
  //  for (int i = 0; i < n; i++) {
  //    int k = orderBlocks[i].size();
  //    orderBlocks.push_back(vector<PrimeOrderPtr>(orderBlocks[i].begin() + (k / 2), orderBlocks[i].end()));
  //    orderBlocks[i].erase(orderBlocks[i].begin() + (k / 2), orderBlocks[i].end());
  //  }
  //}

  State initState(bestState);
  auto initialOrderBlocks(orderBlocks);
  ll initScore = bestScore;
  REP(i_, 20) {
    orderBlocks = initialOrderBlocks;
    auto subBestState(initState);
    ll subBestScore = initScore;
    double start = timer.elapsedMs();
    while (timer.elapsedMs() - start < 50) {
      ...
      if (subBestScore < nowScore) {
        subBestScore = nowScore;
        subBestState = state;
      }
      else {
        swap(orderBlocks[i], orderBlocks[j]);
    ...
    if (bestScore < subBestScore) {
      bestState = subBestState;
