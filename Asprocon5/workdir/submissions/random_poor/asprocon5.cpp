...
struct Bom {
  ...
  vector<SubResources> s_to_sub_resources; // size: ñäÑêî - 1
};
...
void solve() {
  ...
  for (int n = 0; n < R; ++n) {
    ...
    int mainM;
    {
      ...
      mainM = std::next(it, rnd.nextUInt(nm))->first;
      ope.main_m = mainM;
    }
    ...
    for (int s = 0; s < bom.ss; ++s) {
      int& subMs = ope.sub_ms_list[s];
      {
        ...
        subMs = bom.s_to_sub_resources[s].ms_list[rnd.nextUInt(nm)];
      }
