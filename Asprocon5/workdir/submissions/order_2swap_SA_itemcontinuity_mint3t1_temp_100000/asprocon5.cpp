...
int main() {
  ...
  vector<PrimeOrderPtr> porders;
  for (PrimeOrder& o : orders) porders.push_back(&o);
  sort(all(porders), [](const PrimeOrderPtr& a, const PrimeOrderPtr& b) {
    return a->i == b->i ? a->e < b->e : a->i < b->i;
    //return a->e < b->e;
  });
  vector<int> perm;
  for (const PrimeOrderPtr& o : porders) perm.push_back(o->r);
