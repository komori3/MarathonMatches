    ...
    for (int n = 0; n < R; ++n) {
      ...
      int mainM; // choose greedy
      {
        ...
        for (const auto& e : bom.main_resources.m_to_c) {
          ...
          int setupT = 0;
          if (previousI >= 0) setupT = setup_times[mm][pii(previousI, ope.i)];
          int unlockTime = max(order.e, mr.need_setup ? mainMToPreviousT3[mm] : mainMToPreviousT3[mm] - setupT);
...
int main() {
...
#ifdef _MSC_VER
  ifstream ifs("asprocon5/practice/practice04.txt");
  ofstream ofs("asprocon5/practice/practice_output04.txt");
  ...
  State bestState;
  bestState.solveGreedy();
  ll bestScore = bestState.evaluate();
  dump(bestScore);

  while(timer.elapsedMs() < 1900) {
    State state;
    state.solveRandom();
    ll score = state.evaluate();
    if (bestScore < score) {
      bestState = state;
      bestScore = score;
      dump(bestScore);
