...
int main() {
...
#ifdef _MSC_VER
  ifstream ifs("C:\\dev\\MarathonMatches\\ac-heuristics\\AHC001\\tools\\in\\0019.txt");
  istream& in = ifs;
  ofstream ofs("C:\\dev\\MarathonMatches\\ac-heuristics\\AHC001\\tools\\out\\0019.txt");
  ostream& out = ofs;
#else
  istream& in = cin;
  ostream& out = cout;
  ...
  vector<pair<double, SplitSolver::StatePtr>> svec;
  {
    double best = -1.0;
    int loop = 0;
    while (timer.elapsedMs() < time_split) {
