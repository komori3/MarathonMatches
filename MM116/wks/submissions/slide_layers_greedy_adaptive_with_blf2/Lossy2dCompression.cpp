...
struct Board {
  ...
  Board() {}
  Board(double P, int N, int H, int W, const vector<vector<string>>& grids) : P(P), N(N), H(H), W(W), T(0), grids(grids), total_diff(0) {
    diffs.resize(H, vector<int>(W, 0));
    for (const auto& grid : grids) add_layer(grid, 0, 0);
    set_base();
  }
  Board(double P, int N, const vector<vector<string>>& grids, BLF& blf) : P(P), N(N), T(0), grids(grids), total_diff(0) {
    H = blf.strip_height;
    W = blf.strip_width;
    diffs.resize(H, vector<int>(W, 0));
  ...
  void set_base(int r, int c) {
    ...
    int cnt = 0;
    for (int n = 0; n < N; n++) {
      if (layers[n][r][c]) chars[cnt++] = layers[n][r][c];
    }
    if (cnt) {
      sort(chars, chars + cnt);
      // â‘Î•Î·˜a‚Í’†‰›’l‚ÅÅ¬‰»‚³‚ê‚é
      char med = chars[cnt / 2];
      for (int i = 0; i < cnt; i++) {
        char ch = chars[i];
        int diff = abs(ch - med);
        diffs[r][c] += diff;
...
public:
  vector<string> findSolution(double P, int N, vector< vector<string> > grids) {
    ...
    vector<Board> boards(10);
    for (int ext = 0; ext < 10; ext++) {
      boards[ext] = Board::get_random_board(P, N, H + ext, W + ext, grids);
      for (int i = 0; i < N; i++) {
    ...
    //dump(timer.elapsed_ms());
    {
      ...
      Board& board = boards[best_ext];
      while (timer.elapsed_ms() < 9500) {
        for(int i = 0; i < N; i++) {
          board.adjust_layer(i);
          if (timer.elapsed_ms() > 9500) break;
        }
      }
      double score = board.get_score();
      if (score < best_score) {
        best_board = board;
        best_score = score;
        //dump(best_ext, best_score);
      }
    }

    //for(int ext = 0; ext <= 6; ext += 2) {
