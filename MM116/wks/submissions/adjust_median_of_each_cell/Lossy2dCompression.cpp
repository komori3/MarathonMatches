...
struct Board {
  ...
  int T;
  int diff;
  Board(double P, int N, int H, int W, const vector<vector<string>>& grids) : P(P), N(N), H(H), W(W) {
...
public:
  vector<string> findSolution(double P, int N, vector< vector<string> > grids) {
    ...
    int maxWidth = 0;
    for (auto grid : grids) {
      maxHeight = max(maxHeight, (int)grid.size());
      maxWidth = max(maxWidth, (int)grid[0].size());
    }
    Board board(P, N, maxHeight, maxWidth, grids);
    return board.get_ans();
  }
};
