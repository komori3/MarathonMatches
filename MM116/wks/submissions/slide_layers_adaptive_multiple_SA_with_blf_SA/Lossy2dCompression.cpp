...
struct BLF {
  ...
  void build() {
    ...
    int* overlap_ = new int[strip_height * strip_width];
    int** overlap = new int* [strip_height];
    for (int i = 0; i < strip_height; i++) overlap[i] = &overlap_[i * strip_width];

    for (const Rect& rect : rects) {
      memset(overlap_, 0, sizeof(int) * strip_height * strip_width);
  ...
  vector<string> findSolution(double P, int N, vector< vector<string> > grids) {
    ...
    // layer moving part
    {
      ...
      //dump(timer.elapsed_ms());
      {
        ...
        double layer_limit = 9800;
        int loopcnt = 0;

        double layer_start = timer.elapsed_ms();
        int num_interval = 3;
        double interval = (layer_limit - layer_start) / num_interval;

        for (int i = 0; i < num_interval; i++) {
          //dump(i);
          double start = timer.elapsed_ms();
          double end = layer_start + (i + 1) * interval;
          double T = end - start;
          const unsigned RR = (1 << 30), mask = (1 << 30) - 1;

          Board board = Board::get_random_board(P, N, H + best_ext, W + best_ext, igrids);
          while (timer.elapsed_ms() < end) {
