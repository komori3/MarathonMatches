...
struct Board {
  ...
  Board(double P, int N, int H, int W, const vector<vector<vector<int>>>& grids) : P(P), N(N), H(H), W(W), total_grids(0), grids(grids), total_diff(0) {
    ...
    for (int i = 0; i < N; i++) {
      sizes.emplace_back((int)grids[i].size(), (int)grids[i][0].size());
  ...
  Board(double P, int N, const vector<vector<vector<int>>>& grids, BLF& blf) : P(P), N(N), total_grids(0), grids(grids), total_diff(0) {
    ...
    for (int i = 1; i < blf.fixed.size(); i++) {
      ...
      sizes.emplace_back(rect.h, rect.w);
      offsets.emplace_back(rect.y, rect.x);
  ...
  void annealing(double T) {
    ...
    while (timer.elapsed_ms() - start < T) {
      double prev_score = get_score();
      int id = rnd.next_int(N);
      int h, w; tie(h, w) = sizes[id];
      int pr, pc; tie(pr, pc) = offsets[id];
      int nr = rnd.next_int(H - h + 1);
      int nc = rnd.next_int(W - w + 1);

      move_layer(id, nr, nc);

      double now_score = get_score();
...
public:
  vector<string> findSolution(double P, int N, vector< vector<string> > grids) {
    ...
    /*if(P < 0.35)*/ {
      ...
      int initH = board.H;
      for (int i = initH; i >= max(initH - 5, H + 1); i--) {
