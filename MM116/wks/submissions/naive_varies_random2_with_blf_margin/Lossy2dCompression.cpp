...
struct Rect {
  ...
  Rect() {}
  Rect(int id, int w, int h, int ow, int oh) : id(id), w(w), h(h), ow(ow), oh(oh), x(-1), y(-1) {}
  Rect(int id, int x, int y, int w, int h, int ow, int oh) : id(id), x(x), y(y), w(w), h(h), ow(ow), oh(oh) {}
  Rect get_nfp(const Rect& other) const {
    return Rect(-1, x - other.w + 1, y - other.h + 1, w + other.w - 1, h + other.h - 1, -1, -1);
  }
...
struct BLF {
  ...
  pii get_original_size() {
    int original_strip_height = 0;
    int original_strip_width = 0;
    for (const auto& rect : fixed) {
      if (!rect.w) continue;
      if (rect.y + rect.oh > original_strip_height) {
        original_strip_height = rect.y + rect.oh;
      }
      if (rect.x + rect.ow > original_strip_width) {
        original_strip_width = rect.x + rect.ow;
      }
    }
    return { original_strip_height, original_strip_width };
  }
#ifdef _MSC_VER
...
public:
  vector<string> findSolution(double P, int N, vector< vector<string> > grids) {
    ...
    for (int margin = 0; margin < min(H, W); margin++) {
      int area = 0;
      vector<Rect> rects;
      for (int i = 0; i < grids.size(); i++) {
        int id = i;
        const auto& grid = grids[i];
        int oh = grid.size(), ow = grid[0].size();
        int h = max(1, oh - margin), w = max(1, ow - margin);
        area += h * w;
        rects.push_back(Rect(id, w, h, ow, oh));
      }
